...
function editData(button) {
    ...
    let emailInput =
        prompt("Enter the updated email:",
            emailCell.innerHTML);
    let deptInput =
        prompt("Enter the updated  dept:",
            deptCell.innerHTML
        );
    let branchInput =
        prompt("Enter the updated branch:",
            branchCell.innerHTML
        );
    let DOBInput =
        prompt("Enter the updated DOB:",
            DOBCell.innerHTML
        );
...
function deleteData(button) {
    ...
    // Extract the ID or any other unique identifier from the row (adjust this based on your table structure)
    let id = row.cells[0].innerHTML; // Assuming the ID is in the first cell
